...
class Algo :
    ...
    def read_csv(self):
        ...
        return self.time_period,print("Timeslots crées")

    def run_algo(self):
        print("L'algorithme travaille et simule une batterie de ", self.capacity_max_batt, " Wh et un rendement de ", self.batt.efficiency)
        for ts in self.time_period:
            ts.reset()

        # Identification des périodes de déficit et de surplus et les croisements de courbe
        periode_de_deficit = []
        in_deficit = False
        last_index_def = 0
        ...
        for ts in self.time_period: # s'assurer que toutes les valeurs sont bien compiler dans les timeslots
            ts.compute()

        depassement_capacite = 0 # permet de savoir combien de fois la vitesse de chargement limite le modèle

        # identifier si on commence par un surplus ou un déficit
        first_ts = self.time_period[0]
        if first_ts.production > first_ts.consumption:

        # ----------------------- si le premier timeslot à un surplus de production------------------------------------
        # ------------------------------------------------------------------------------------------------------------
            for i in range(len(periode_de_surplus)):
                surplus = periode_de_surplus[i]
                deficit = periode_de_deficit[i]

                for ts in surplus: # allocation du surplus en fonction de l'espace restant dans la batterie et de la vitesse de chargement
                ...
                # seulement l'allocation du PV stocké dans la batterie aux timeslots les plus coûteux
                k = 1
                ...
                # décharge réelle et allocation en même temps
                ...
                for td in deficit:
                    ...
                    if td.allocated == False and td.buy_before == False and td.for_later == False:
                        ...
                        while k <= len(new_def):
                            ...
                            # déterminer la plage d'achat possible pour le timeslot sur lequel on travaille
                            candidats_prix = new_def[:idx_imp]
                            k += 1
                            if len(candidats_prix) == 0:
                                continue
                            l = 1
                            # décision d'achat anticipé
                            while l <= len(candidats_prix):
                                ...
                                need_internal = ts_imp.deficit / self.batt.efficiency
                    ...
                    # décharge et charge réelle de la batterie
                    self.batt.discharge(td.wh_from_PV_in_batt)
                    self.batt.discharge(td.wh_buy_before)
                    self.batt.charge(td.wh_buy_for_later)

        # ----------------------- si le premier timeslot à un deficit de production------------------------------------
        # ------------------------------------------------------------------------------------------------------------
        else:
            deficit = periode_de_deficit[0]
            soc_ctrl = 0
        # on travaille sur la première periode de déficit
            for td in deficit:
                index = deficit.index(td)
                if td.allocated == False and td.buy_before == False and td.for_later == False:
                    new_def = deficit[index:]
                ...
                self.batt.discharge(td.wh_buy_before)
                self.batt.charge(td.wh_buy_for_later)
            # on travaille sur toutes les périodes
            for i in range(len(periode_de_surplus)):
                surplus = periode_de_surplus[i]
        ...
        # calcul de la différence entre ce qu'on voulait acheter en avance et ce qui a vraiment été acheté
        diff = 0
        average_price = 0
        ...
        print("La vitesse de chargement a limité l'achat anticipé ",depassement_capacite,"fois")
