...
class Algo :
    ...
    def run_algo(self):
        ...
        for ts in self.time_period:
            # auto-consommation
            cout_autoconso_cap.append(ts.autoconsumption * ts.cost_prod)
            wh_autoconso_cap.append(ts.autoconsumption)
            # produit et stocké
            cout_prod_stock_cap.append(ts.wh_from_PV_in_batt * (ts.cost_batt + ts.cost_prod))
            ...
            cout_wanted_buy_before.append(ts.wh_buy_before*ts.price)
            # kWh vraiment achetés en avance
            cout_really_buy_before_cap.append(ts.wh_buy_for_later *(ts.cost_batt + ts.price))
            wh_really_buy_for_later_cap.append(ts.wh_buy_for_later)
            # kWh achetés au réseau en direct
            cout_grid_direct_cap.append(ts.solde_to_buy * ts.price)
            wh_grid_direct_cap.append(ts.solde_to_buy)
            # vente au réseau
            gain_vente_prod_cap.append(ts.wh_sell_to_grid * ts.price_sell_to_grid)
